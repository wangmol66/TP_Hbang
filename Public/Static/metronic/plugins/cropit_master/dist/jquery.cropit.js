...
/***/ function(module, exports) {
	...
	var ERRORS = {
	  IMAGE_FAILED_TO_LOAD: { code: 0, message: '图片加载失败.' },
	  SMALL_IMAGE: { code: 1, message: '图片太小.' }
	};
...
//自定义插件
var cropitMaster = function(option){

	var modalObj = null;

	option.uploadBtn.click(function(){

		if($('#cropit-master-modal').length == 0){
			var modal = '<div id="cropit-master-modal" class="modal fade modal_small" tabindex="1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">';
				modal += '<div class="modal-dialog">';
				modal += '<div class="modal-content">';
				modal += '<div class="modal-header">';
				modal += '<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>';
				modal += '<h4 class="modal-title">上传图片</h4>';
				modal += '</div>';
				modal += '<form action="" class="form-horizontal" id="add-form" method="post">';
				modal += '<div class="modal-body">';
				modal += '<div class="image-editor">';
				modal += '<button class="btn btn-default btn-sm select-img-btn" type="button"><i class="fa fa-search"></i> 选择图片</button>';
				modal += '<input type="file" class="cropit-image-input hide">';
				modal += '<div class="cropit-preview" style="margin:30px;height:'+option.height+';width:'+option.width+'"></div>';
				modal += '<div class="image-size-label">';
				modal += '调整图片大小';
				modal += '</div>';
				modal += '<input type="range" class="cropit-image-zoom-input">';
				modal += '<button class="rotate-ccw" type="button"><i class="fa fa-rotate-left"></i></button>';
				modal += '<button class="rotate-cw" type="button"><i class="fa fa-rotate-right"></i></button>';
				modal += '</div>';
				modal += '</div>';
				modal += '<div class="modal-footer">';
				modal += '<button class="btn green export" type="button"><i class="fa fa-save"></i> 确定</button>';
				modal += '<button class="btn default" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i> 关闭</button>';
				modal += '</div>';
				modal += '</form>';
				modal += '</div>';
				modal += '</div>';
				modal += '</div>';
				$('body').append(modal);
				modalObj = $('#cropit-master-modal');
				modalObj.find('.image-editor').cropit({
			          imageBackground: true,
			          imageBackgroundBorderWidth: 30,
			          onImageError:function(obj, code, message){
			          	alert(obj.message);
			          }
			   });

				modalObj.find('.select-img-btn').click(function() {
					modalObj.find('.cropit-image-input').click();
					});

				modalObj.find('.rotate-cw').click(function() {
					modalObj.find('.image-editor').cropit('rotateCW');
			    });
				modalObj.find('.rotate-ccw').click(function() {
					modalObj.find('.image-editor').cropit('rotateCCW');
			     });

				modalObj.find('.export').click(function() {
		          	  var imageData = modalObj.find('.image-editor').cropit('export');
		          	  option.complete && option.complete(option.uploadBtn, imageData);
		          	  modalObj.modal('hide');
		        });
		}	

		modalObj.find('.image-editor').find('input[type="file"]').val('');
		modalObj.find('.image-editor').find('img').attr('src', '');
		modalObj.modal();
