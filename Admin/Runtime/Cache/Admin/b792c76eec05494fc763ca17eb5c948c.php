      ...
      <div class="row">
        <div class="col-md-12"> 
          ...
          <ul class="page-breadcrumb breadcrumb">
            <li> <a href="javascript:void(0);"> 往期回顾 </a> <i class="fa fa-angle-right"></i> </li>
...
<script>
    ...
    var Editer={
         ...
         * @returns {string}
         */
        getTree2:function (arr,a) {
            var str='';
            for(var i=0;i<arr.length;i++) {
            	var selected = ''
            	for(var n=0;n<this.treeConf.def.length;n++){
	            	if(arr[i][this.treeConf.key] == this.treeConf.def[n]){
	            		console.log(arr[i][this.treeConf.key],this.treeConf.def);
	            		selected = 'selected'
	            	}
            	}
                if (typeof(arr[i]) == 'object') {
                    if(typeof(arr[i][this.treeConf.child]) =='object'){
                        if(arr[i][this.treeConf.pkey]=='0'){
                            str+='<option '+(selected)+' value="'+arr[i][this.treeConf.key]+'">'+arr[i][this.treeConf.name]+'</option>'+this.getTree(arr[i][this.treeConf.child],2);
                        }else{
                            str+='<option '+(selected)+' value="'+arr[i][this.treeConf.key]+'">'+this.space(a)+arr[i][this.treeConf.name]+'</option>'+this.getTree(arr[i][this.treeConf.child],a+2);
                        }
                    }else{
                        str+='<option '+(selected)+' value="'+arr[i][this.treeConf.key]+'">'+this.space(a)+arr[i][this.treeConf.name]+'</option>';
                    }
                }
            }
            return str;
									...
									.ajax(
											'<?php echo U("getCategoryInfo");?>',
											{
												...
												success : function(data,
													...
													model.find('.modal-title').html('编辑分类  - '+ data.pos_name);
