...
str;
    ...
    public function index(){
        ...
        $this->assign('__sidebar', 'Addons/index');
        $this->display();
    }
    ...
    public function getLists(){
    	...
    	$tA = $this->pagination(D('Hooks'), array('where' => 'id = 0','order' => 'id desc'));
    	
		$list = D('Addons')->getList();
		$total =  $list ? count($list) : 1;
		$tA['data'] = array_slice($list, I('iDisplayStart'), $tA['iDisplayLength']);
    	$tA['iTotalRecords'] = $total;
    	$tA['iTotalDisplayRecords'] = $total;
		...
		if( ! empty($tA['data'])){
			...
			foreach ($tA['data'] as $k => $v){
				...
				$btn = '';
//				$btn = '<a href="'.U('edithook?id='.$v['id']).'" 
//					   class="btn default  btn-xs blue-stripe">设置</a>';
				
				$btn = '<a href="'.U('edithook?id='.$v['id']).'" 
					     class="btn default  btn-xs blue-stripe">禁用</a>';
				$btn .= ' <a href="'.U('delhook?id='.$v['id']).'" 
						class="btn ajax-request-btn default btn-xs red-stripe" alert-message="确认是否卸载？">卸载</a>';
				$tA['aaData'][$k][] = $v['title'];
				$tA['aaData'][$k][] = $v['name'];
				$tA['aaData'][$k][] = $v['description'];
				$tA['aaData'][$k][] = $v['status_text'];
				$tA['aaData'][$k][] = $v['author'];
				$tA['aaData'][$k][] = $v['version'];
//				$tA['aaData'][$k][] = $btn;
			}	
		}
    ...
    public function hooks(){
        ...
        //加载静态插件
        $this->assign('__plugins', 'datatable');
        $this->assign('__sidebar', 'Addons/hooks');
    ...
    public function hooksList(){
    	...
    	$tA = $this->pagination(D('Hooks'), array('order' => 'id desc'));	
		if( ! empty($tA['data'])){
			int_to_string($tA['data'], array('type'=>C('HOOKS_TYPE')));
			foreach ($tA['data'] as $k => $v){
    ...
    public function updateHook(){
        ...
        if($data){
            if($data['id']){
                ...
                if($flag !== false){
                    S('hooks', null);
                    $this->success('更新成功', U('Addons/hooks'));
                }else{
                    $this->error('更新失败');
                }
            }else{
                ...
                if($flag){
                    S('hooks', null);
                    $this->success('新增成功', U('Addons/hooks'));
                }else{
                    $this->error('新增失败');
                }
            }
