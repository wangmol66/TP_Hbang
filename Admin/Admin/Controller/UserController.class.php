...
class UserController extends AdminController {
    ...
    public function index(){
        ...
        //加载静态插件
        $this->assign('status', $this->_getStatus());
    ...
    public function setPassword(){
    	$this->meta_title = '更新密码';
    	//加载静态插件
    	$this->assign('status', $this->_getStatus());
    	$this->assign('__plugins', 'datatable,uniform');
    	$this->assign('__sidebar', 'Config/group');
    	$this->display();
    }
    ...
    public function setnewPassword(){
    	...
    	$tA = M('ucenter_member')->where(array('password'=>$this->think_ucenter_md5($old, UC_AUTH_KEY)))->find();

    	if(!$tA){
    		$this->error('密码错误');
    	}
    	
    	$query = M('ucenter_member')->where(array('id'=>1))->save(array('password'=>$this->think_ucenter_md5($password, UC_AUTH_KEY)));
    ...
    public function getLists(){
    	...
    	$status = trim(I('status'));
    	 ...
    	 $option = array('order' => 'uid desc', 'where' => array('status' => array('NEQ', -1)));
    	 if($uid){
			 $option['where']['uid'] = $uid;   	 
    	 }
    	 
    	 if($name){
    	 	 $option['where']['nickname'] = array('LIKE', '%'.$name.'%');   	 
    	 }
    	 ...
    	 $tA   = $this->pagination('Member', $option);
    	...
    	if( ! empty($tA['data'])){
			foreach ($tA['data'] as $k => $v){
				...
				$btn = '<div class="btn-group">
									<a href="'.U('/AuthManager/group/uid/'.$v['uid']).'" class="btn btn-default btn-xs">
									...
									<ul role="menu" class="dropdown-menu pull-right">
										...
										<li>
											<a href="'.U('User/changeStatus/method/deleteUser/id/'.$v['uid']).'" class="ajax-request-btn" alert-message="确认是否删除'.$v['nickname'].'？">
												 删除
											</a>
										</li>
									</ul>
				...
				$tA['aaData'][$k][] = $v['score'];
				$tA['aaData'][$k][] = $v['login'];
				$tA['aaData'][$k][] = $v['last_login_time'] ? time_format($v['last_login_time']) : '';
				$tA['aaData'][$k][] = $v['last_login_ip'] ? long2ip($v['last_login_ip']) : '';
...
//	}
    ...
    public function updateNickname(){
        ...
        $this->thisaddress = array('用户','修改昵称');
    ...
    public function updatePassword(){
        $this->thisaddress = array('用户','修改密码');
    ...
    public function action(){
        ...
        $this->thisaddress = array('用户','用户行为');
    ...
    public function addAction(){
        $this->thisaddress = array('用户','新增行为');
    ...
    public function editAction(){
        ...
        $this->thisaddress = array('用户','编辑行为');
